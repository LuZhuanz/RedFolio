...
RedFolio 是一个桌面红利持仓应用，用于记录 A 股股票、场内 ETF 和场外 ETF 联接基金的买入/卖出流水，计算股息率、预计本年税前红利收入，并展示持仓结构。
...
- SQLite 本机数据存储
- AKShare 自动拉取 A 股、场内 ETF 和 ETF 联接基金行情/净值及分红数据
...
- 支持 A 股股票、沪深场内 ETF 和场外 ETF 联接基金。
- 交易流水手动录入，行情、净值和分红手动点击刷新。
